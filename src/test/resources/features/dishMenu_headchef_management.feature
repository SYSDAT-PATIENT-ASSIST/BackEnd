Feature: Menu management by head chef
  ...
  Scenario: Remove a dish from the menu
    Given the dish "Pasta Bolognese" exists in the menu
    When I click on "Pasta Bolognese"
    And I click the "Fjern" button
    And I confirm the removal in the dialog
    Then the dish "Pasta Bolognese" should be removed from the menu
    And I should see the message "Retten er blevet fjernet."
    And "Pasta Bolognese" should no longer be visible in the menu list

  Scenario: Edit an existing dish
    Given the dish "Rugbrød med æg og rejer" exists in the menu
    When I click on "Rugbrød med æg og rejer"
    And I click the "Rediger" button
    And I change the "Titel" to "Rugbrød med håndpillede rejer"
    And I click the "Gem" button
    Then the system should validate the updated input
    And the dish title should be updated to "Rugbrød med håndpillede rejer"
    And I should see the message "Ændringer er gemt."

  Scenario: Show error when mandatory fields are missing during edit
    Given the dish "Grøntsagslasagne" exists in the menu
    When I click on "Grøntsagslasagne"
    And I click the "Rediger" button
