...
class DishControllerTest {
    ...
    @Test void createNewDish() {
        DishDTO input = new DishDTO(); input.setName("TestDish");
        DishDTO created = new DishDTO(); created.setName("TestDish");
    ...
    @Test void getDishById_notFound() {
        when(ctx.pathParam("id")).thenReturn("1");
    ...
    @Test void deleteExistingDish_deleted() {
        when(ctx.pathParam("id")).thenReturn("1");
    ...
    @Test void deleteExistingDish_notDeleted() {
        when(ctx.pathParam("id")).thenReturn("1");
    ...
    @Test void getFilteredDishes_noParams() {
    ...
    @Test void getFilteredDishes_withStatus() {
        when(ctx.queryParam("status")).thenReturn(DishStatus.TILGÆNGELIG.toValue());
        when(ctx.queryParam("allergen")).thenReturn(null);
        List<DishDTO> dishes = Collections.singletonList(new DishDTO());
    ...
    @Test void getFilteredDishes_withAllergen() {
        when(ctx.queryParam("status")).thenReturn(null);
    ...
    @Test void getFilteredDishes_withStatusAndAllergen() {
        when(ctx.queryParam("status")).thenReturn(DishStatus.TILGÆNGELIG.toValue());
        when(ctx.queryParam("allergen")).thenReturn("GLUTEN");
        List<DishDTO> dishes = Collections.singletonList(new DishDTO());
    ...
    @Test void getFilteredDishes_invalidParam() {
    ...
    @Test void updateDishName_success() {
        when(ctx.pathParam("id")).thenReturn("1");
        when(ctx.body()).thenReturn("NewName");
        DishDTO updated = new DishDTO(); updated.setName("NewName");
    ...
    @Test void updateDishName_notFound() {
        when(ctx.pathParam("id")).thenReturn("1");
        when(ctx.body()).thenReturn("NewName");
    ...
    @Test void updateDishAvailability_missingDates() {
        when(ctx.pathParam("id")).thenReturn("1");
        DishDTO input = new DishDTO(); // missing dates
        when(ctx.bodyAsClass(DishDTO.class)).thenReturn(input);
        controller.updateDishAvailability(ctx);
        verify(ctx).status(400);
    ...
    @Test void createDishWithRecipeAndIngredients_success() {
        DishDTO input = new DishDTO();
        input.setName("Dish");
        input.setAllergens(Set.of(Allergens.GLUTEN));
    ...
    @Test void updateDishStatus_success() {
        when(ctx.pathParam("id")).thenReturn("1");
        String statusValue = DishStatus.TILGÆNGELIG.toValue();
        when(ctx.body()).thenReturn(statusValue);
        DishDTO updated = new DishDTO(); updated.setStatus(DishStatus.TILGÆNGELIG);
    ...
    @Test void updateDishAllergens_success() {
        when(ctx.pathParam("id")).thenReturn("1");
    ...
    @Test void updateDishRecipeAndAllergens_success() {
        when(ctx.pathParam("id")).thenReturn("1");
        DishDTO input = new DishDTO();
    ...
    @Test void getMostOrderedDishes_customLimit() {
    ...
    @Test void getMostOrderedDishes_defaultLimit() {
    ...
    @Test void getAvailableDishes_noAllergen() {
        when(ctx.queryParam("allergen")).thenReturn(null);
        List<DishDTO> list = Collections.singletonList(new DishDTO());
    ...
    @Test void getAvailableDishes_withAllergen() {
        when(ctx.queryParam("allergen")).thenReturn("GLUTEN");
        List<DishDTO> list = Collections.singletonList(new DishDTO());
    ...
    @Test void getAvailableDishes_invalidAllergen() {
